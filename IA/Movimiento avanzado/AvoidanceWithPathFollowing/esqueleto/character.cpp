...
Character::Character() : mLinearVelocity(0.0f, 0.0f), mAngularVelocity(0.0f)
{
    ...
    mSteerings.push_back(new PathFollowing(this));
    mWeights.push_back(0.4f);
...
void Character::OnUpdate(float step)
{
    ...
    for (Steering* it : mSteerings)
    {
        ...
        {
            ResultSteering iter;
            iter.linear += it->GetSteering(USVec2D()).linear * mWeights.at(index);
            iter.angular += it->GetSteering(0.f).angular * mWeights.at(index);
            steeringResult += iter;
            ++index;
        }
