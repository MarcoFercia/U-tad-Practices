...
ResultSteering& PathFollowing::GetSteering(const USVec2D& _target)
{
    ...
    if (mCharacter && mCharacter->GetPathfinder()->IsPathCompleted())
    {
        ...
        Path::PointSegment Segment = mCharacter->GetPathfinder()->GetPath()->GetClosestPoint(mProjection);
        mClosest = Segment.SegmentPoint;
        float LookAhead = mCharacter->GetParams().look_ahead;
        mTarget = mCharacter->GetPathfinder()->GetPath()->GetPointAhead(Segment, LookAhead);
